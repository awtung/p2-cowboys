"""Power2 algorithm contained within a class """
...
class Power2:
    ...
    def calc_series(self):
        #multiplier = 1
        limit = self._series
        f = [2]  # Power2 starting array/list
        while limit > 0:
            self.set_data(f[0])
            f = [f[0] ** 2 ]
            limit -= 1
...
if __name__ == "__main__":
    ...
    print(f"Power number for {n} = {Power2.number}")
    print(f"Power series for {n} = {Power2.list}")

    '''Using method to get data from object'''
    for i in range(n):
        print(f"Power sequence {i + 1} = {power2.get_sequence(i)}")
